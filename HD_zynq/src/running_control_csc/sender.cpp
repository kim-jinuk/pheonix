...
constexpr int META_SIZE = OBJECTINFO_SIZE * OBJECT_COUNT;
...
// boos library
UdpSender::UdpSender(const std::string& ip, int port)
    : io_context_(), socket_(io_context_), frame_id(0), endpoint_(boost::asio::ip::make_address(ip), port) {
    socket_.open(udp::v4());
...
void UdpSender::UdpSend(const std::vector<std::vector<uint8_t>>& packets) {
    ...
    std::cout << "[ASIO] Frame " << frame_id << " sent with " << packets.size() << " async packets\n";
    frame_id++;
}
