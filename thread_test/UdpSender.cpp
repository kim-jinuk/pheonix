...
// 상수 정의
constexpr int HEADER_SIZE = 12;  // magic(4) + frame_id(4) + packet_id(2) + total_packets(2)
constexpr int NUM_OBJECTS = 5;
constexpr int META_SIZE = 2 + sizeof(ObjectInfo) * NUM_OBJECTS;  // nx, ny + ObjectInfo
...
std::vector<std::vector<uint8_t>> UdpSender::BuildUdpPackets(const std::vector<ObjectInfo>& objs, uint8_t nx, uint8_t ny) {
    ...
    for (int i = 0; i < total_packets; ++i) {
        ...
        // Magic Word
        uint32_t magic_word = htonl(0xDEADBEEF);  // fixed sync value
        memcpy(&packet[0], &magic_word, 4);

        // Frame ID
        uint32_t fid_net = htonl(frame_id);
        memcpy(&packet[4], &fid_net, 4);

        // Packet ID & Total Packets
        ...
        for (int j = 0; j < NUM_OBJECTS; ++j) {
            ...
            memcpy(&packet[14 + j * sizeof(ObjectInfo)], &obj, sizeof(ObjectInfo));
        }
